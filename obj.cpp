...
bool objloader::loadmtl(const char *mtlfilename, const char *objfilename) {
  ...
  while (fgets(currline, LINE_SZ, mtlfile)) {
    ...
    else fprintf(stderr, "obj: unknown command : %s in material file %s at line %i, \"%s\"\n",
...
bool objloader::loadobj(const char *filename) {
  ...
  if (objfile == NULL) {
    fprintf(stderr, "obj: error reading file: %s\n", filename);
    return false;
  }
  ...
  while (fgets(currline, LINE_SZ, objfile)) {
    ...
    // parse objects
      ...
      if (loadmtl(mtlfilename, filename) == 0) {
        fprintf(stderr, "obj: error loading %s\n", mtlfilename);
        return false;
      }
      continue;
    ...
    else fprintf(stderr, "obj: unknown command: %s in obj file %s at line %i, \"%s\"\n",
...
bool obj::load(const char *filename) {
  ...
  loopv(loader.facelist) {
    ...
    if (face == NULL) {
      fprintf(stderr, "obj: null face for %s\n", filename);
      return false;
    }
    if (face->vertexnum > 4) {
      fprintf(stderr, "obj: too many vertices for %s\n", filename);
      return false;
    }
    int v[] = {0,0,0,0};
  ...
  if (!allposset) fprintf(stderr, "obj: some positions are unspecified for %s\n", filename);
  if (!allnorset) fprintf(stderr, "obj: some normals are unspecified for %s\n", filename);
  if (!alltexset) fprintf(stderr, "obj: some texture coordinates are unspecified for %s\n", filename);
...
#undef COPY_FIELD
  ...
  fprintf(stderr, "obj: %s, %i vertices\n", filename, vertnum);
  fprintf(stderr, "obj: %s, %i groups\n", filename, grpnum);
  fprintf(stderr, "obj: %s, %i materials\n", filename, matnum);
  return true;
}
