...
let loadmtl filename =
  ...
  let do_mtl_line mat line =
    ...
    match tokens with
      ...
      | "newmtl"::name::[] -> append mat;
        { default_material with name=name }
...
let loadobj filename =
  ...
  let do_obj_line line =
    ...
    let parse_face f =
      let append v0 v1 v2 =
        tris := {
          v = {xi=parse_vertex v0; yi=parse_vertex v1; zi=parse_vertex v2};
          m = !currmat
        } :: !tris in
      match f with
    ...
    let parse_line l = match l with
      | "v" :: rest -> parse_vec3 pos rest
      | "vn" :: rest -> parse_vec3 nor rest
      | "vt" :: rest -> parse_vec3 tex rest
      | "f" :: rest -> parse_face rest
      | "mtllib" :: filename :: [] -> materials := loadmtl filename
      | _ -> () (* ignore the rest, including comments and groups *)
    in parse_line tokens
  in
